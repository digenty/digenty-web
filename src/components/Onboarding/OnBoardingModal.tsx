...
//   const { clearDraft: clearSchoolDraft } = useFormDraft("onboarding:school", schoolFormik.values, v => schoolFormik.setValues(v));
//   const { clearDraft: clearBranchDraft } = useFormDraft("onboarding:branch", branchFormik.values, v => branchFormik.setValues(v));
...
import { Spinner } from "../ui/spinner";
import { SchoolOverview } from "./SchoolOverview";
import { CreateSchoolTypes, DRAFT_KEY_BRANCH, DRAFT_KEY_SCHOOL, DRAFT_KEY_STEP } from "./types";
import { WelcomeInputs } from "./WelcomeInputs";
import { useIsMobile } from "@/hooks/useIsMobile";
...
function clearDrafts(): void {
  [DRAFT_KEY_SCHOOL, DRAFT_KEY_BRANCH, DRAFT_KEY_STEP].forEach(k => localStorage.removeItem(k));
}
...
const OnboardingModal = ({ initialShow }: OnboardingModalProps) => {
  const [step, setStep] = useState<number>(() => {
    const saved = readDraft<number>(DRAFT_KEY_STEP);
    return saved ?? 1;
  });
  const [showModal, setShowModal] = useState(initialShow);
  const isMobile = useIsMobile();
  ...
  const schoolFormik = useFormik<CreateSchoolTypes>({
    initialValues: readDraft<CreateSchoolTypes>(DRAFT_KEY_SCHOOL) ?? defaultSchoolValues,
    validationSchema: schoolSchema,
    onSubmit: values => {
      mutateSchool(values, {
        onSuccess: data => {
          saveDraft(DRAFT_KEY_SCHOOL, values);
          toast({
            title: "School created successfully",
            description: data.message,
            type: "success",
          ...
          const nextStep = 2;
          setStep(nextStep);
          saveDraft(DRAFT_KEY_STEP, nextStep);
        },
        onError: error => {
          toast({
            title: error.message ?? "Something went wrong",
  ...
  const branchFormik = useFormik<BranchFormValues>({
    initialValues: readDraft<BranchFormValues>(DRAFT_KEY_BRANCH) ?? defaultBranchValues,
    onSubmit: async values => {
      if (!values.activeOption) {
        toast({ title: "Please select a branch option", type: "error" });
        return;
