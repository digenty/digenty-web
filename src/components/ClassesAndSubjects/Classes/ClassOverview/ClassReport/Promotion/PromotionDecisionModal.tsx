            ...
            <label key={opt.id} className="flex items-center gap-3 cursor-pointer group">
              <div className="relative flex items-center justify-center">
                <input
                  ...
                  className="peer appearance-none size-4 rounded-full border border-border-strong checked:border-bg-state-primary checked:bg-bg-state-primary transition-all pointer-events-none"
                />
                <div className="absolute size-1.5 rounded-full bg-white opacity-0 peer-checked:opacity-100 transition-opacity pointer-events-none" />
              </div>
              <span className="text-text-subtle text-sm font-normal group-hover:text-text-default transition-colors">
                {opt.label}
              </span>
            </label>
          ))}
        ...
        <>
          <div className="flex flex-col gap-2">
            ...
            <Select value={selectedClassId} onValueChange={(value) => {
              ...
              <SelectTrigger className="bg-bg-input-soft border-none h-11 w-full">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Select value={selectedArmId} onValueChange={(value) => {
              const arm = arms?.data?.find((arm: Arm) => arm.id.toString() === value);
              setSelectedArmId(value);
              setArmToPromoteTo(arm?.name);
              }} disabled={!selectedClassId}>
              <SelectTrigger className="bg-bg-input-soft border-none h-11 w-full capitalize">
  ...
  const footerActions = (
    <div className="flex items-center justify-between w-full">
      ...
      <Button
        ...
        className="bg-bg-state-primary hover:bg-bg-state-primary-hover! text-white h-7! px-4 text-sm font-medium"
      >
