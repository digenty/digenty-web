...
export default function SubmitScoreModal({ open, onOpenChange, onSubmit, isSubmitting, classArm, subjectName }: SubmitScoreModalProps) {
  const handleSubmit = () => {
    onSubmit("SUBMITTED", onOpenChange);
  };
    ...
    <>
      <Modal
        ...
        ActionButton={
          ...
          >
            {isSubmitting && <Spinner className="text-text-white-default" />}
            Submit Request
          </Button>
        }
      >
        <div className="flex flex-col gap-4 px-5 py-4">
          ...
          <div className="border-border-default bg-bg-basic-blue-subtle rounded-sm border px-3 py-2">
            <p className="text-text-subtle text-sm font-normal">
              Once submitted, you will no longer be able to make changes. To edit later, you&apos;ll need to request edit access from the class
              teacher or admin.
            </p>
          </div>
        </div>
