...
export default function Subject({ title, classes }: SubjectProps) {
  ...
  return (
    <div className="p-4 md:px-8 md:py-4">
      ...
      <div className="flex-start bg-bg-muted flex w-full flex-col gap-4 rounded-sm px-3 py-2 md:w-219">
        <h2 className="text-text-default text-md font-semibold md:ml-4">{title}</h2>
          ...
          {classes.map(cl => {
            ...
            return (
              <li key={cl.id} className="border-border-default border-b-1">
                <div className="flex flex-col gap-2 p-2 md:flex-row md:items-center md:justify-between md:px-6 md:py-3">
                  <div className="flex flex-col gap-2">
                    <p className="text-text-default text-md font-semibold">{cl.grade}</p>
                    <Badge className={`border-border-default rounded-sm border p-1 text-xs font-medium ${statusUpdate}`}>{cl.subjectStatus}</Badge>
                  </div>

                  <div>
                    {cl.subjectStatus === "Not Started" ? (
                      <Button className="bg-bg-state-primary hover:bg-bg-state-primary text-text-white-default h-7 w-24 rounded-sm px-2 py-1">
                        Enter Score
                      </Button>
                    ) : (
                      ""
                    ...
                    {cl.subjectStatus === "Submitted" ? (
                      <div className="flex gap-1 md:justify-between">
                        <Button
                          onClick={handleOpen}
                          className="border-border-darker text-text-default bg-bg-state-secondary h-7 rounded-sm border px-2 py-1 text-sm"
                        >
                          <Question fill="var(--color-icon-default-muted)" /> Request Edit Access
                        </Button>
                        <Button className="border-border-darker text-text-default bg-bg-state-secondary h-7 rounded-sm border px-2 py-1 text-sm">
