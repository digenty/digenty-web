...
import { buildFeeItemPayload, initialFeeItemValues, useFeeFormData, type FeeItemFormValues } from "./useFeeForm";
...
export const AddFee = () => {
  ...
  const { sessionId } = useFeeFormData();
  const { mutate: createFeeItem, isPending } = useCreateFeeItem();
  const addAnotherRef = useRef(false);

  const initialValues: FeeItemFormValues = {
    ...initialFeeItemValues,
    sessionId: sessionId ?? "",
  };

  const handleSubmit = (values: FeeItemFormValues, helpers: FormikHelpers<FeeItemFormValues>) => {
    createFeeItem(buildFeeItemPayload(values), {
      onSuccess: () => {
      ...
      onError: (error: unknown) => {
        const message = (error as { message?: string })?.message ?? "Failed to create fee";
        toast.error(message);
      },
      onSettled: () => {
