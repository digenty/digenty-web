...
export const FeeItemDetail = () => {
  ...
  const { data: rawItemData, isLoading } = useGetFeeItemById(itemId);
  const { mutate: deleteFeeItem, isPending: isDeleting } = useDeleteFeeItem();
  const [openDeleteModal, setOpenDeleteModal] = useState(false);

  const item = ((rawItemData as { data?: FeeItemApiItem })?.data ?? rawItemData) as FeeItemApiItem | undefined;

  const itemName = item?.feeName ?? `Fee Item #${itemId}`;
  const termLabel =
    (item as { termLabel?: string; term?: string } | undefined)?.termLabel ??
    (item as { termLabel?: string; term?: string } | undefined)?.term ??
    "—";
  const amount = item?.amount ?? 0;
  const quantity = item?.quantity ?? 1;

  const appliedBranches: AppliedBranch[] =
    (item as { branches?: { branchName: string; classes: { className: string; type?: string; amount: number }[] }[] } | undefined)?.branches?.map(
      b => ({
        title: b.branchName,
        classes: b.classes?.map(c => ({ className: c.className, type: c.type ?? "Class", amount: c.amount })) ?? [],
      }),
    ) ?? [];
  ...
  return (
    <div className="flex items-center justify-center px-4 py-3 pb-8 md:px-8">
      {isLoading && <Skeleton className="bg-bg-input-soft h-100 w-full max-w-full" />}
        ...
        <div className="flex flex-col gap-4 px-6 py-4">
          ...
          <div className="bg-bg-basic-orange-subtle border-border-default flex items-center gap-3 rounded-md border px-3 py-2">
            ...
            <div className="text-text-subtle text-sm">
              Deleting this fee item will remove it from the fee setup and it will no longer appear in new invoices. Once removed, this action cannot
            ...
            <div className="border-border-default grid grid-cols-2 rounded-sm border md:grid-cols-4">
              <div className="border-border-default flex flex-col gap-4 border-r border-b p-3 py-4 md:border-b-0 md:p-6">
                ...
                <Badge className="bg-bg-badge-green text-bg-basic-green-strong border-border-default h-5 rounded-md text-xs font-medium">
                  {(item as { active?: boolean } | undefined)?.active === false ? "Inactive" : "Active"}
                </Badge>
              </div>
              <div className="border-border-default flex flex-col gap-4 border-r border-b p-3 py-4 md:border-b-0 md:p-6">
              ...
              <div className="border-border-default flex flex-col gap-4 border-r p-3 py-4 md:p-6">
                ...
                <div className="text-text-default text-md font-medium">₦{amount}</div>
              </div>
