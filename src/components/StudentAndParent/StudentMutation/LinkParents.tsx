...
export const LinkParents = ({
  ...
  const { data, isPending } = useGetParents({ limit: 50, branchId: branchSelected?.id, search: debouncedSearchQuery });
  const parents = data?.pages.flatMap(page => page.content) ?? [];

  const removeParent = (id: number) => {
    const prnts = selectedParents.filter(parent => parent.id !== id);
    setSelectedParents(prnts);
  };
            ...
            {!data || isPending ? (
              ...
              <div className="border-border-default h-75 space-y-3 overflow-y-auto rounded-sm border p-3">
                {parents.map((parent: Parent) => {
                  ...
                  return (
                    <div key={parent.id} className="bg-bg-card border-border-darker flex items-center gap-3 rounded-lg border p-4">
                      <Checkbox
                        checked={!!isChecked}
                ...
                {parents.map((parent: Parent) => {
                  ...
                  return (
                    <div key={parent.id} className="bg-bg-card border-border-darker flex items-center gap-3 rounded-lg border p-4">
                      ...
                      />
                      <div className="flex items-center gap-2">
                        <Avatar className="size-10 md:size-8" url={parent.image ?? ""} />
                        <div className="">
                          <p className="text-text-default text-sm font-medium">
                            {parent.firstName} {parent.lastName}
                          </p>
