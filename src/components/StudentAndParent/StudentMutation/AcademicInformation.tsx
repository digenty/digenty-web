...
export const AcademicInformation = ({
  ...
  useEffect(() => {
    if (isBranchRestricted && !branchId && visibleBranches?.data?.length) {
      const assignedBranch = visibleBranches.data[0].branch;
      formik.setFieldValue("branchId", assignedBranch.id);
      setBranchId(assignedBranch.id);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isBranchRestricted, branchId, visibleBranches]);
        ...
        <div className="space-y-2">
          ...
          {!visibleBranches || loadingBranches ? (
            ...
            <Select
              ...
              value={branches?.data.find((branch: BranchWithClassLevels) => branch.branch.id === branchId)?.branch.uuid}
              onValueChange={value => {
