...
export const LinkEntity = ({ entity, open, setOpen }: { entity: "Students" | "Parents"; open: boolean; setOpen: (open: boolean) => void }) => {
  ...
  return (
    <Modal
      className="bg-bg-card block max-w-200! overflow-y-auto"
      open={open}
      setOpen={setOpen}
      title={`Link ${entity}`}
      ActionButton={
    ...
    >
      ...
      <div className="bg-bg-subtle border-border-default space-y-2 border-t px-4 py-3">
        ...
        <div className="flex flex-wrap gap-2">
          {/* <div className="border rounded-full bg-bg-badge-default border-border-default text-xs text-text-subtle p-1 flex items-center gap-1">
                  <Avatar username="Damilare John" className="size-3.5" />
                  <span>Damilare John</span>
                  <XIcon className="size-3.5 text-icon-default-muted cursor-pointer" />
                  </div>
