    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent className="bg-bg-card border-border-default text-text-default w-48 py-2.5 shadow-sm">
        <DropdownMenuItem
          ...
          onClick={() => router.push(`/staff/invoices/${row.original.invoiceId}`)}
        >
...
export const columns: ColumnDef<Invoice>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span className="text-text-muted cursor-pointer text-sm font-normal">
        {row.original.issuedDate ? new Date(row.original.issuedDate).toLocaleDateString() : "—"}
      </span>
    ),
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => <span className="cursor-pointer">{getBadge(formatInvoiceStatus(row.original.status))}</span>,
  },
  {
    id: "actions",
    header: () => <div />,
