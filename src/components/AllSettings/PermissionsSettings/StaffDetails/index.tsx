      ...
      {data && !isPending && !isError && (
        <div className="flex w-full items-center justify-center px-4 py-4 md:mx-auto md:max-w-250 md:px-8 md:py-6">
          <div className="flex w-full flex-col gap-8">
            ...
            {showEdit === true && <>{<TeacherAssignments teacherName={data.data.fullname} staffId={Number(staffId)} setShowEdit={setShowEdit} />}</>}

            {hasExistingAssignments && showEdit === false && (
              <>
                {data.data.branches.map((branch: StaffBranch) => (
                  <div key={branch.branchId} className="border-border-default gap-4 rounded-md border p-4">
                    <div className="border-border-default flex items-center justify-between gap-3 border-b pb-4">
                      ...
                      <Button
                        ...
                        className="bg-bg-state-secondary! hover:bg-bg-state-secondary-hover! text-text-default border-border-darker rounded-md border"
                      >
                        <Edit fill="var(--color-icon-default-muted)" className="size-4" /> Edit
                      </Button>
                    </div>
                            ...
                            <div key={idx} className="bg-bg-muted flex flex-col gap-2 rounded-md px-4 py-2">
                              <div className="itesm-center flex gap-2">
                                <div className="bg-bg-badge-gray flex size-5 items-center justify-center rounded-sm p-1">
                                  {permission.moduleName === "Classes & Subjects" && (
                                    <GraduationCap fill="var(--color-icon-default-subtle)" className="size-3" />
                                  )}
                                  {permission.moduleName === "Student & Parent Records" && (
                                    <Group fill="var(--color-icon-default-subtle)" className="size-3" />
                                  )}
                                  {permission.moduleName === "Attendance" && (
                                    <CalendarCheck fill="var(--color-icon-default-subtle)" className="size-3" />
                                  )}
                                  {permission.moduleName === "Settings" && <Settings4 fill="var(--color-icon-default-subtle)" className="size-3" />}
                                </div>
