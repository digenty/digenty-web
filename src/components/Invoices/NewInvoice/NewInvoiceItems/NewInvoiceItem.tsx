...
export const NewInvoiceItem = ({ formik }: Props) => {
  ...
  const updateItem = (id: string, data: Partial<InvoiceItem>) => {
    formik.setFieldValue("items", items.map(i => (i.id === id ? { ...i, ...data } : i)));
  };
  ...
  const removeItem = (id: string) => {
    formik.setFieldValue("items", items.filter(i => i.id !== id));
  };
  ...
  return (
    <div className="">
            ...
            <TableBody className="[&_tr:first-child]:pt-5">
              {items.map(item => (
                <TableRow key={item.id} className="flex w-full flex-row items-center gap-2 border-none px-4 py-2">
                  ...
                  <td className={cn("bg-bg-input-soft flex h-8! w-full max-w-78 items-center rounded-md", (Array.isArray(formik.errors.items) ? (formik.errors.items as Record<string, string>[])?.[items.indexOf(item)]?.name : false) && "border-border-destructive border")}>
                    <Input
                      type="text"
      ...
      {typeof formik.errors.items === "string" && (
        <p className="text-text-destructive mt-2 text-xs font-light">{formik.errors.items}</p>
      )}
