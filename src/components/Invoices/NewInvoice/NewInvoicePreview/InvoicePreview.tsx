...
const formatDate = (d: Date | null) => (d ? new Date(d).toLocaleDateString("en-GB", { day: "2-digit", month: "2-digit", year: "numeric" }) : "—");
...
export const InvoicePreview = ({ onPreviewToggle, formik }: ToggleProp) => {
  ...
  const recipientLabel =
    values.billTo.length === 0 ? "—" : values.billTo.length === 1 ? firstRecipient.name : `${firstRecipient.name} +${values.billTo.length - 1} more`;
    ...
    <div className="bg-bg-subtle border-border-default rounded-sm border p-1.5">
      <div className="border-border-default bg-bg-default flex flex-col gap-6 rounded-sm border p-5">
        ...
        <div>
          ...
          <div className="bg-bg-muted text-text-default rounded-sm p-4 text-sm font-normal">{values.note || "No note added."}</div>
        </div>
