  ...
  return (
    <div className="mt-6 flex w-full flex-col gap-6">
            ...
            <SheetContent className="bg-bg-card border-border-default mt-1.5 mr-1.5 h-[98vh] w-[97vw] rounded-md border md:w-130 [&>button]:hidden">
              ...
              <div className="m-0 flex h-screen place-content-center items-center justify-center">
                <div className="flex flex-col items-center gap-4">
                  ...
                  <SheetClose asChild>
                    <Button className="border-border-darker bg-bg-state-secondary! text-text-default h-7 w-14.5 rounded-sm border px-2 py-1 text-sm">Close</Button>
                  </SheetClose>
                </div>
              </div>
            </SheetContent>
      ...
      <div className="space-y-2">
        ...
        <div className={cn("bg-bg-input-soft flex h-9 items-center rounded-md px-3", touched.amount && errors.amount && "border-border-destructive border")}>
        ...
        {touched.amount && errors.amount && (
          <p className="text-text-destructive text-xs font-light">{errors.amount as string}</p>
        )}
      </div>
        ...
        <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
          ...
          <PopoverContent className="bg-bg-card border-border-default border p-0">
            <Calendar
              ...
              onSelect={d => { setFieldValue("transactionDate", d ?? null); setFieldTouched("transactionDate", true); setCalendarOpen(false); }}
      ...
      <div className="border-border-default w-full border-b pb-6">
        <Select>
          ...
          <SelectContent className="bg-bg-card border-border-default text-text-default border">
            <SelectGroup>
              <SelectItem value="1"><Avatar className="size-4" /> <span>Damilare John</span></SelectItem>
              <SelectItem value="2"><Avatar className="size-4" /> <span>Damilare John</span></SelectItem>
            </SelectGroup>
          </SelectContent>
        </Select>
      </div>
